...
class Flex implements PluginInterface, EventSubscriberInterface
{
    ...
    public function activate(Composer $composer, IOInterface $io)
    {
        ...
        $repositoryClassesProperty = new \ReflectionProperty(RepositoryManager::class, 'repositoryClasses');
        $repositoryClassesProperty->setAccessible(true);
        $manager = new RepositoryManager($this->io, $this->config, $composer->getEventDispatcher(), $this->rfs);
        foreach ($repositoryClassesProperty->getValue($composer->getRepositoryManager()) as $type => $class) {
            $manager->setRepositoryClass($type, $class);
        }
